...
class Detector:
    ...
    def __init__(self, pnet_param="./param/pnet.pt", rnet_param="./param/rnet.pt", onet_param="./param/onet.pt",
        ...
        self.__image_transform = transforms.Compose([    #图片加载进来转成tensor格式 已经转成chw了；
            transforms.ToTensor()
        ])
    ...
    def __onet_detect(self, image, rnet_boxes):
        ...
        return utils.nms(np.array(boxes), 0.7, isMin=True)#用最小的，因为要把中间框给去了
    ...
    def __pnet_detect(self, image):
        ...
        while min_side_len > 12:
            img_data = self.__image_transform(img)
            if self.isCuda:
                img_data = img_data.cuda() #把图像放到cuda里去
            img_data.unsqueeze_(0)#升维度 增加一个0轴的维度
            ...
            idxs = torch.nonzero(torch.gt(cls, 0.6))#只取置信度大于0.6的值的索引
